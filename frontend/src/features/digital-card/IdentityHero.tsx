...
export function IdentityHero({ profile }: { profile: PublicStudent }) {
  const coverPhoto = profile.school.organizationType === 'club'
    ? (profile.school.coverPhoto || profile.coverPhoto)
    : profile.coverPhoto
  return (
    <section className="digital-card-hero" aria-label={`${profile.name} profile image`}>
      <div className="digital-card-hero__cover">
        {coverPhoto ? <img src={coverPhoto} alt="" /> : <span aria-hidden="true" />}
      </div>
